...
@Component({
  selector: 'app-student-ebook',
  templateUrl: './student-ebook.component.html',
  styleUrls: ['./student-ebook.component.scss']
})
export class StudentEbookComponent implements OnInit {

  tempNewsObj:object={};
  tempChargeObj:object={};
  tempReceicedObj:object={};
  studentEbookArray:any[]=[];
  ...
  getStudentToCourseRegistrationListLedgerId($ledgerID:any){
    this.reportService.fetchStudentToCourseRegistrationReportLedgerId($ledgerID).subscribe(response => {
      ...
      this.getStudentEbookList();
      
    })
  }
  getStudentEbookList(){
    this.reportService.fetchStudentEbookList(this.tempNewsObj).subscribe(response => {
      this.studentEbookArray=response.data;
      console.log("studentEbookArray:",this.studentEbookArray);
    })
  }
